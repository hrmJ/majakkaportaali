  ...
  var Presentation = function () {
    ...
    this.ToggleOpen = function () {
      ...
      if (!abort) {
        ...
        $("#controllink, #addcontentlink, #layoutlink, #slide_type_link")
          .parent()
          .toggle();
        if (wasclosed) {
    ...
    this.SetContent = function () {
      ...
      return $.when(this.LoadSlides()).done(() => {
        ...
        if (
          !Slides.Controls.GetCurrentService().GetControlledByPresentation()
        ) {
    ...
    this.LoadControlsAndContent = function () {
      ...
      if (!this.initial_load_ready) {
        ...
        this.controls = {
          ...
          textcontentadder: new Slides.Widgets.ContentAdders.TextContentAdder(
            this
          ),
          biblecontentadder: new Slides.Widgets.ContentAdders.BibleContentAdder(
            this
          ),
          songcontentadder: new Slides.Widgets.ContentAdders.SongContentAdder(
            this
          ),
        ...
        this.controls.backgroundchanger = new Slides.Widgets.StyleWidgets.BackgroundChanger(
          this
        );
        this.controls.backgroundchanger.LoadLocalBackgrounds();
        this.controls.backgroundchanger.InitializeEvents();

        this.controls.fontchanger = new Slides.Widgets.StyleWidgets.FontChanger(
          this
        );
        this.controls.fontchanger.CreateFontControllers();
        Slides.Styles.Controller.UpdateControllers(this);

        this.controls.positionchanger = new Slides.Widgets.StyleWidgets.PositionChanger(
          this
        );
        this.controls.positionchanger.CreateControllers();

        this.controls.layoutloader = new Slides.Widgets.StyleWidgets.LayoutLoader(
          this
        );
    ...
    this.Move = function (where) {
      ...
      if ($target.length == 0)
        $target = this.$section[where]().find("article:" + first_or_last);
      if ($target.length > 0) {
    ...
    this.Update = function () {
      ...
      return $.when(this.SetContent()).done(() => {
        var new_msg = new Utilities.Message(
            "Diaesitys päivitetty",
            this.$controller_window
          ),
          $sec = undefined,
          $slide = undefined;
